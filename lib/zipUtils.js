...
/**
 * Tạo tên file .md theo format tiếng Việt không dấu viết liền
 * Ví dụ: "Hướng dẫn mẫu.md" -> "huong-dan-mau.md"
 */
function generateVietnameseSlugFilename(originalFilename) {
  // Loại bỏ extension .md
  const nameWithoutExt = originalFilename.replace(/\.md$/i, '');
  
  // Chuyển thành không dấu, lowercase
  const noAccent = removeVietnameseAccents(nameWithoutExt).toLowerCase();
  
  // Thay khoảng trắng và ký tự đặc biệt bằng dấu gạch ngang
  const slug = noAccent
    .replace(/[^a-z0-9\s-]/g, '') // Chỉ giữ lại chữ cái, số, khoảng trắng, dấu gạch ngang
    .trim()
...
/**
 * Thu thập tất cả các file ảnh từ toàn bộ ZIP
 * Tìm tất cả file ảnh bất kể cấu trúc thư mục
...
function collectAttachments(zip) {
  ...
  zip.forEach((relativePath, file) => {
    // Chỉ lấy file (không phải thư mục) và là file ảnh
    if (!file.dir && getMimeType(relativePath)) {
      ...
      // Luôn thêm tên file thuần túy (không có path) để fallback
      const filename = relativePath.split('/').pop();
      if (filename !== normalizedPath) {
        attachments.set(filename, file);
      }
    }
  });
...
export async function processZip(file) {
  try {
    ...
    return {
      finalMarkdown,
      originalMarkdownName: generateVietnameseSlugFilename(markdownFilename)
    };
  } catch (error) {
